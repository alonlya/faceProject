...
def warp_and_crop_face(src_img,
    ...
    return face_img
class FACE():
    def __init__(self, bbox, ldmk=None, score=None):
        self.bbox = bbox
        self.ldmk = ldmk
        self.score = score

def get_aligned_faces(img, bboxsnlandmarks, w, h):
    faces = []
    ...
    for i in range(len(bboxsnlandmarks)):
        landmark = bboxsnlandmarks[i][4:-2]
        facial5points = [[landmark[j * 2]*w, landmark[j * 2 + 1]*h] for j in range(5)]
...
def recover_pad_output(outputs, pad_params, score_th):
    """recover the padded output effect"""
    img_h, img_w, img_pad_h, img_pad_w = pad_params
    outputs = outputs[outputs[:, 15] > score_th]
